...
1. **Prompt engineering.** Extraction prompts live in `extraction/prompts/` as versioned `.txt` files. Never hardcode prompt text in Python. Each iteration increments the version number. Old versions are preserved.
...
3. **Validation.** Every extracted graph must pass through `extraction/validator.py`. Invalid graphs are logged but don't halt the batch — extraction continues and failures are manually reviewed.

4. **Model comparison.** The comparison experiment (`extraction/model_comparison/run_comparison.py`) runs the same 10 transcripts through all candidate models. Scoring uses a 5-criterion rubric (R1–R5). The winner gates all scale extraction.
...
| `extraction/tagger.py` | Speaker-tags transcripts before extraction |
| `extraction/extractor.py` | Main extraction loop with caching and retry |
| `extraction/validator.py` | Structural constraint enforcement |
| `extraction/model_comparison/run_comparison.py` | 3-model comparison experiment |
...
- Cache everything. Check `data/graphs/free_text/{tid}.json` before any API call.
- Prompts are versioned files. Load from `extraction/prompts/v{n}.txt`.
...
- Failed extractions are written to `extraction/failed.txt`.
...
1. `tagger.py` — speaker-tag transcripts → `data/tagged/*.jsonl`
2. `extractor.py` — extract graphs → `data/graphs/free_text/*.json`
