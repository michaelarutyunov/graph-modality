...
- **`classification/fusion/models.py`** — Classifier zoo with 4 architectures
  (single, stacked, gated, late) — all consume frozen modality embeddings.

- **`classification/fusion/run.py`** — Config-driven experiment runner.
  Any architecture × any modality combination × any target.

- **`encoding/gnn/autoencoder.py`** — Self-supervised GIN encoder.
