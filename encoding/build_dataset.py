...
from encoding.gnn.encode import encode_graphs
...
def _load_modality_arrays() -> dict[str, tuple[np.ndarray, list[str]]]:
    ...
    gin_embs, gin_ids = encode_graphs()
    print(f"  GIN: {gin_embs.shape}, {len(gin_ids)} IDs")
...
def build_dataset() -> dict[str, Any]:
    ...
    # ── Load all modalities ─────────────────────────────────────────────────
    modalities = _load_modality_arrays()
    ...
    metadata = {
        ...
        "source_encoders": {
            ...
            "graph": "GIN autoencoder (self-supervised, node type reconstruction, 128-dim)",
        },
        "targets": {
            "cohort": cohort_info,
            "ai_adoption": ai_info,
